...
type Props = {
  ...
  loading: boolean
  status: any
  filter: 'all' | 'pending' | 'sent' | 'bounced' | 'unsubscribed'
  onFilterChange: (f: 'all' | 'pending' | 'sent' | 'bounced' | 'unsubscribed') => void
...
export default function AudienceList({ contacts, selectedIdx, searchQ, loading, status, filter, onFilterChange, onSearch, onToggle, onSelectAll, onUpload, onEdit, onDelete, onBack, onReview, onAddClick, onScanBounces, scanningBounces }: Props) {
  ...
  const filtered = contacts.filter(c => filter === 'all' || (filter === 'pending' ? c.status !== 'sent' && c.status !== 'bounced' && c.status !== 'unsubscribed' : c.status === filter));
  ...
  return (
    <div className="rounded-2xl border border-slate-200/80 bg-white shadow-sm">
      ...
      <div className="flex items-center gap-1 border-b border-slate-100 px-6 py-2">
        {(['all', 'pending', 'sent', 'bounced', 'unsubscribed'] as const).map((f: 'all' | 'pending' | 'sent' | 'bounced' | 'unsubscribed') => (
          <button key={f} onClick={() => onFilterChange(f)}
            ...
            {f === 'all' ? 'All' : f === 'pending' ? '⏳ Pending' : f === 'sent' ? '✅ Sent' : f === 'bounced' ? '❌ Bounced' : '🚫 Unsub'}
          </button>
        ))}
      </div>
