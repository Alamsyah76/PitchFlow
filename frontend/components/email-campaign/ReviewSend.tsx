    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">Double-check before sending</p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm font-semibold text-slate-900">{selectedCount} contacts</p>
            </div>
          </div>
        </div>
        <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
          {sendResult ? (
            <div className="flex items-center gap-3">
              <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${sendResult.startsWith('Sent') ? 'bg-emerald-50' : 'bg-red-50'}`}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none"
                  stroke={sendResult.startsWith('Sent') ? '#059669' : '#dc2626'} strokeWidth="1.5" strokeLinecap="round">
                  {sendResult.startsWith('Sent') ? <polyline points="20 6 9 17 4 12"/> : <><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></>}
                </svg>
              </div>
              <div>
                <p className="text-xs font-medium text-slate-500">Status</p>
                <p className={`text-sm font-semibold ${sendResult.startsWith('Sent') ? 'text-emerald-700' : 'text-red-700'}`}>{sendResult}</p>
              </div>
            </div>
          ) : (
